...
import util.profileDB as profileDB
import secrets
import bcrypt
import json

def add_paths(router):
    router.add_route(Route("GET", "/static/functions.js", js))
    router.add_route(Route("GET", "/static/style.css", style))
    router.add_route(Route("GET", "/image", images))
    router.add_route(Route("POST", "/register", registration))
    router.add_route(Route("POST", "/home", newlogin))    
...
def js(request, handler):
    send_file("public/static/functions.js", "text/javascript; charset=utf-8", request, handler)

def style(request, handler):
    send_file("public/static/style.css", "text/css; charset=utf-8", request, handler)

def images(request, handler):
    path_prefix= "/image/"
    image_name = request.path[request.path.find(path_prefix)+len(path_prefix):]
    image_name = image_name.replace("/", "")
    send_file("public/image/"+ image_name, "image/jpeg", request, handler)
